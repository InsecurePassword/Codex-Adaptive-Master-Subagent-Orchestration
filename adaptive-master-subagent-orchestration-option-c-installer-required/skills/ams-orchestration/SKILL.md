...
Apply this project-agnostic orchestration contract to the current top-level objective. It coexists with all higher-priority instructions and project-native workflows, state, validation, and tooling, and assumes no fixed repository layout, methodology, work-unit format, or subagent count.
...
This lean runtime contains no profile bootstrap, repair, migration, or installation procedure. Before delegation, verify that required managed profiles are discoverable. If any required profile is missing or malformed, do not create or modify it from this skill; continue only with safe master-owned work and report that the deterministic package installer must be run.

Before delegation, inspect the effective loaded custom-agent registry. Prefer reusable profiles under `$CODEX_HOME/agents/` (normally `~/.codex/agents/`); use `<project-root>/.codex/agents/` only when global profiles are unavailable, isolated overrides are required, or the project intentionally provides them. Do not duplicate effective global profiles locally.

The expected Sol/Terra/Luna profiles are the Cartesian product of families `sol`, `terra`, and `luna` with efforts `low`, `medium`, `high`, `xhigh`, and `max`, named `ams_<family>_<effort>`. Optional Spark profiles are `ams_spark_low`, `ams_spark_medium`, and `ams_spark_high`.

Resolve only model identifiers and reasoning efforts actually available to the current runtime. Never invent an identifier or misstate the observed model/effort. Map Light to `low` and Extra High to `xhigh`. Spark unavailability or an unsupported Spark effort must not block or weaken normal orchestration.
...
If a needed managed profile is missing or malformed, continue safely with an available truthful profile or master execution and report that the deterministic package installer must be rerun; do not bootstrap from this runtime skill.
...
1. Read the complete handoff, original objective, acceptance criteria, applicable `AGENTS.md` files, skills, policies, plans, prompt-runner state, and other authoritative instructions.
2. Inspect the live repository and environment. Verify branch, `HEAD`, working tree, user changes, referenced commits, worktrees, state files, artifacts, tests, and validation rather than assuming the handoff is current.
3. Reconcile the handoff with observed state. Preserve unrelated user work. Treat prior child reports and completion claims as evidence, not proof.
...
5. Resume from the earliest unfinished or unverified dependency affecting completion. Treat the recorded next action as a hint and correct it when live evidence requires.
6. Do not wait for inaccessible or terminated threads. Recover useful work from the repository, commits, worktrees, logs, artifacts, and durable state. Replace agents only when the current task graph and intensity justify it.
7. Use the project's existing task/state mechanism; do not create a competing ledger when an equivalent exists.
8. Begin executing immediately. Do not return only a status summary, plan, or rewritten handoff while authorized executable work remains.

### Continued orchestration

Apply the resolved intensity to each ready stage while retaining sole master authority over the actual zero-to-many topology. Do not recreate the old roster by default. Continue to use bounded work orders, non-overlapping ownership, independent master verification, proportional independent review, safe parallelism, and normal Sol/Terra/Luna/Spark routing. Continue safe unblocked work when another branch is blocked.

Request user input only when a required decision cannot be resolved from authoritative evidence or a safe reversible assumption and no meaningful unblocked progress remains.

### Recovery anti-stall and completion

A pause marker, handoff, commit, checkpoint, clean tree, completed phase, passed validation subset, empty worker set, completed immediate next action, or ability to produce another handoff is not terminal while mandatory work remains. At every boundary, update existing durable state when appropriate, identify the next required ready work, and execute or delegate it immediately.

Declare completion only when the original objective and mandatory acceptance criteria are satisfied; every required task is complete, superseded, or transparently externally blocked; no required ready work or unresolved deviation remains; accepted child work is verified and integrated; required validation passed or unavoidable failures are accurately reported; final architecture, security, regression, scope, risk, and final-diff review is complete at the warranted level; project state and documentation are consistent; and all relevant child results are collected and threads closed.

The final response must include selected intensity and source, completed work, material decisions, subagent usage, validation evidence, repository/commit state when relevant, and genuine blockers or residual risks. Do not expose private chain-of-thought.
