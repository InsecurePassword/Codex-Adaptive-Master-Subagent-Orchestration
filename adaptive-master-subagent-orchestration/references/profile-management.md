...
Read completely only for profile selection, verification, deployment, or an explicitly requested profile operation. The package ships 18 canonical profiles under `assets/agent-profiles/`; installers deploy them to the effective Codex agent registry.
...
The packaged defaults are `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, and `gpt-5.3-codex-spark`. Treat them as requested routes, not proof of account availability or observed runtime identity.

## Canonical profile contract

The exact bundled bytes define each managed name, description, model, reasoning effort, bounded non-root instructions, and Codex V2 guidance overrides. All families use the same permission-neutral field shape:
...
Managed profiles must not set `sandbox_mode`, approval policy, network access, writable roots, tool grants, provider credentials, or other permission overrides. Permissions come only from higher-priority platform/user policy and the current work order. Spark has no exclusive privilege and remains worker-only.
...
- Spark accepts only `worker`/`none`;
