...
- `enabled`, `allow_implicit_invocation`, `project_governance`, `spark_enabled`, `spark_available`: Boolean;
...
Reject duplicate or unknown keys, unsupported schemas, coercion, invalid TOML, invalid types, unsafe paths, or extra tables. A valid schema-2 file created before `project_governance` existed may omit only that key; resolve it as `true` in memory and persist it on the next authorized project-settings write. All other keys remain required. Normalize stored `moderate` to runtime `balanced` and retain `moderate` when persisting schema 2.

Global persistence is manual only. AMS never creates, modifies, repairs, migrates, or deletes the global file. If both settings files are absent in a trusted stable project, initialize the exact disabled project default. Never persist controls in an untrusted, trust-indeterminate, or rootless context.
...
- `AMS STATUS`: report canonical project/global paths, safety/validity, effective source, effective mode, governance, Spark controls, profile management, and the exact activation blocker. Search no unrelated configuration.
...
On disable, intensity/governance/profile/Spark change, trust loss, package transition, user interruption, or material plan correction:
