...
description: Install or repair managed profiles when needed, then run or resume Sol Max-led adaptive zero-to-many direct-child orchestration.
---
...
If a required managed profile is missing or malformed, use `<plugin-root>/scripts/bootstrap_profiles.py` to install or repair only package-managed profiles. Run a dry-run before repair or upgrade when practical. Preserve unrelated user-authored profiles and back up managed files before replacement.

When all required selected profiles are valid and discoverable, profile bootstrap is a strict no-op. Do not repeat bootstrap during normal project execution.
