...
OPTION='A'
EXPECTED_SKILLS=['profile-manager', 'runtime']
...
def check_manifest():
    ...
    assert data["name"]=="adaptive-master-subagent-orchestration-option-a-two-skill"
...
def check_skills():
    ...
    mapping={"profile-manager":"ams-installer","runtime":"ams-orchestration","resume":"ams-orchestration","unified":"adaptive-master-subagent-orchestration"}
    ...
    if OPTION=="A": assert "ams-installer" in texts and "bootstrap_profiles.py" in texts["ams-installer"]
    if OPTION=="B":
    ...
    if OPTION=="C": assert "ams-installer" not in texts
